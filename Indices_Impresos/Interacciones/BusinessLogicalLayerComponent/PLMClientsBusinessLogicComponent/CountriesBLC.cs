...
namespace PLMClientsBusinessLogicComponent
{
    public class CountriesBLC
    {
        ...
        public List<CountryMobileInfo> getCountriesByPrefixByTarget(string prefix, PLMClientsBusinessEntities.Catalogs.TargetOutputs targetId)
        {
            List<CountryMobileInfo> countriesMobile = PLMClientsDataAccessComponent.CountriesDALC.Instance.getCountriesByPrefixByTarget(prefix, (byte)targetId);

            //Obtiene la edición correspondiente al páis:
            this.addEditionByPrefix(countriesMobile, prefix, targetId);

            //Se agrega el menú correspondiente a cada páis:
            this.addMenuesByPrefix(countriesMobile, prefix, targetId);
        ...
        private void addEditionByPrefix(List<CountryMobileInfo> countriesMobile, string prefix, PLMClientsBusinessEntities.Catalogs.TargetOutputs targetId)
        {
            foreach (CountryMobileInfo country in countriesMobile)
            {
                MobileEditionInfo edition = PLMClientsBusinessLogicComponent.EditionsBLC.Instance.getByPrefixByTargetByCountry(prefix, (byte)targetId, country.ID);
        ...
        private void addMenuesByPrefix(List<CountryMobileInfo> countriesMobile, string prefix, PLMClientsBusinessEntities.Catalogs.TargetOutputs targetId)
        {
            foreach (CountryMobileInfo country in countriesMobile)
                country.Menues = PLMClientsDataAccessComponent.MenuesDALC.Instance.getByPrefixByTargetByEdition(country.ISBN, prefix, (byte)targetId);
        }
