...
namespace PLMClientsBusinessLogicComponent
{
    public class EventCategoriesBLC
    {
        ...
        #region Public Methods
            ...
            public List<PLMClientsBusinessEntities.EventCongressInfo> getEvents(byte typeId, byte categoryId, string organizer)
            {
                return PLMClientsDataAccessComponent.EventDALC.Instance.getEvents(typeId, categoryId, organizer);
            }

            //public List<PLMClientsBusinessEntities.EventCongressInfo> getEventsCategory(byte typeId, byte categoryId)
            //{
            //    return PLMClientsDataAccessComponent.EventDALC.Instance.getEventsCategory(typeId, categoryId);

            //}

            public List<PLMClientsBusinessEntities.EventCongressDetailInfo> getCongress(byte typeId)
            {
                return PLMClientsDataAccessComponent.EventDetailDALC.Instance.getCongress(typeId);
            }

            public List<PLMClientsBusinessEntities.EventCongressDetailInfo> getOrganizer(byte categoryId, byte typeId)
            {
            ...
            public int insert(EventCongressDetailInfo BEntity, int userId, string hash)
            {
            ...
            public void update(PLMClientsBusinessEntities.EventCongressDetailInfo businessEntity, int userId, string hash)
            {
            ...
            public void deleteEventAddress(EventCongressDetailInfo bussinesEntity)
            {
                PLMClientsDataAccessComponent.EventDetailDALC.Instance.delete(bussinesEntity);

            }

            public PLMClientsBusinessEntities.EventCongressInfo getEvent(int eventId)
            {
                return PLMClientsDataAccessComponent.EventDALC.Instance.getOne(eventId);
            }

            public PLMClientsBusinessEntities.EventCongressDetailInfo getEvents(int eventId)
            {
