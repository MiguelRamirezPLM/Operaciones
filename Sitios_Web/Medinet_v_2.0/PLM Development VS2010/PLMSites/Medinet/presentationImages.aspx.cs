...
public partial class productImages : System.Web.UI.Page
{
    ...
    public static double anchoMayor,largoMayor,hei;
    ...
    protected override void OnInit(EventArgs e)
{
    ...
    protected void imgBtnBackProducts_Click(object sender, ImageClickEventArgs e)
    {
        ...
        this.Response.Redirect("classifiedProductShots.aspx");
    }
    ...
    protected void addNewImage(object sender, EventArgs e)
    {
        ...
        {
            if (ddlPresentations.SelectedValue == "-1")
                ScriptManager.RegisterStartupScript(this, typeof(Page), "messageScript", "jAlert('Selecciona una presentacion, por favor.', 'Presentacion no valida');", true);
            else
            {
                ...
                string NombreImagen = rowIndexx + "_" + presentationId + "_" + this._brand + "_" + divisionName + "_" + File1.FileName;
                MedinetBusinessEntries.PresentationImages Image = new MedinetBusinessEntries.PresentationImages();
                presentationId = Convert.ToInt32(ddlPresentations.SelectedValue);
                imageId = 1;
                Image.productShot = NombreImagen;
                Image.Active = 1;
                Image.presentationId = presentationId;
                Image.ImageId = imageId;
                Image.ImageSize = firstImage;
                int ImageId = MedinetBusinessLogicComponent.PresentationsBLC.Instance.addImageToPresentation(Image, this._userId, this._hashkey);
    ...
    protected void saveImage(object sender, EventArgs e)
    {
        ...
        presentationId = (int)grdPresentationCatalog.GetRowValues(rowIndex, "PresentationId");
        string divisionName = Session["divisionName"].ToString();
        rowIndex = rowIndex + 1;
        ...
        try
        {
            ...
            MedinetBusinessLogicComponent.PresentationsBLC.Instance.addImageToPresentation(Image, this._userId, this._hashkey);
    ...
    protected void grdDeletingRow_click(object sender, DevExpress.Web.Data.ASPxDataDeletingEventArgs e)
    {
        try
        {
            e.Cancel = true;
            int rowIndex = (int)Session["rowIndex"];
            int presentationId = (int)grdPresentationCatalog.GetRowValues(rowIndex, "PresentationId");
            ...
            row.productShot = lblNames.Text;
            row.ImageId = imageId;
            row.ImageSize = "";

            if (row.PreviousImage != "not.jpg")
                MedinetBusinessLogicComponent.PresentationsBLC.Instance.deleteImageToPresentation(row, this._userId, this._hashkey);
    ...
    private void getPresentationCatalog()
    {
        ...
        List<MedinetBusinessEntries.PresentationDetailInfo> presentationCatalog =
            MedinetBusinessLogicComponent.PresentationsBLC.Instance.getPresentationsByImage(this._editionId, this._divisionId, this._categoryId, this._productId, this._pharmaFormId);
        ...
        {
            this.Sizes = MedinetBusinessLogicComponent.PresentationsBLC.Instance.getSizes();
            if (SizesIm.Rows.Count == 0)
                getImageSizes();
        }
    ...
    {
        this.ddlPresentations.DataSource = MedinetBusinessLogicComponent.PresentationsBLC.Instance.getPresentationList(this._editionId, this._divisionId, this._categoryId, this._productId, this._pharmaFormId);
