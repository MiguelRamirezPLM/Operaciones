IF OBJECT_ID('dbo.plm_spCRUDUsers') IS NOT NULL
	DROP PROCEDURE dbo.plm_spCRUDUsers
go

/* 
	Author:			Juan Ramírez				 
	Object:			dbo.plm_spCRUDUsers
	
	Description:	Perform the basic operations, or in other words CRUD operations.
					Where CRUD stands for:

						C - Create	{0}
						R - Read	{1}
						U - Update	{2}
						D - Delete	{3}

	Company:		PLM Latina.

	DECLARE
		@CRUDType		tinyint,
		@userId			int



	EXECUTE dbo.plm_spCRUDUsers
		@CRUDType,			
		@userId
		
 */ 

CREATE PROCEDURE dbo.plm_spCRUDUsers
(
	@CRUDType			TINYINT,
	@userId				INT,
	@nickName			VARCHAR(40) = NULL,
	@nickPrefixId		INT = NULL,
	@password			VARCHAR(100) = NULL,
	@active				BIT = NULL
)
AS
BEGIN

	-- If @CRUDType belongs to {0,2}, then the parameters shouldn't be null:
	IF (@CRUDType IN (0,2) AND (@nickName IS NULL OR @nickPrefixId IS NULL OR @password IS NULL OR @active IS NULL))
	BEGIN
		RAISERROR ('The current operation requires more parameters', 16, 1)
		RETURN -1
	END

	-- C = {(CREATE, 0)}:
	IF (@CRUDType IN (0))
	BEGIN


		INSERT INTO [dbo].[Users]
           ([NickName]
           ,[NickPrefixId]
           ,[Password]
           ,[Active])
		 VALUES
			   (@nickName
			   ,@nickPrefixId
			   ,@password
			   ,@active)
	 
		RETURN SCOPE_IDENTITY()

	END

	-- R = {(Read, 1)}
	IF (@CRUDType IN (1))
	BEGIN
		
		SELECT [UserId]
			  ,[NickName]
			  ,[NickPrefixId]
			  ,[Password]
			  ,[Active]
		  FROM [dbo].[Users]
		 WHERE [UserId] = @userId			

		RETURN @@ROWCOUNT
	END
	
	-- U = {(Update, 2)}
	IF (@CRUDType IN (2))
	BEGIN

		UPDATE [dbo].[Users]
		   SET [NickName] = @nickName
			  ,[NickPrefixId] = @nickPrefixId
			  ,[Password] = @password
			  ,[Active] = @active
		 WHERE [UserId] = @userId	

		RETURN 0

	END	

	-- D = {(Delete, 3)}:
	IF (@CRUDType IN (3))
	BEGIN
		DECLARE @error int

		BEGIN TRAN
		
		--Delete all entries in the [dbo].[ClientUsers] table:
		DELETE FROM [dbo].[ClientUsers]
		WHERE [UserId] = @userId	

		-- Get the value of the @@ERROR global variable:
		SET @error = @@ERROR

		IF (@error != 0)
		BEGIN
			
			ROLLBACK TRAN 
			RAISERROR ('An error ocurred during deleting the client associated with the user', 16, 1)
			RETURN -1

		END	

		--Delete all entries in the [dbo].[UserFolioCodes] table:
		DELETE FROM [dbo].[UserFolioCodes]
		WHERE [UserId] = @userId	

		-- Get the value of the @@ERROR global variable:
		SET @error = @@ERROR

		IF (@error != 0)
		BEGIN
			
			ROLLBACK TRAN 
			RAISERROR ('An error ocurred during deleting the folio associated with the user', 16, 1)
			RETURN -1

		END	
		
		--Delete all entries in the [dbo].[UserCodes] table:
		DELETE FROM [dbo].[UserCodes]
		WHERE [UserId] = @userId	

		-- Get the value of the @@ERROR global variable:
		SET @error = @@ERROR

		IF (@error != 0)
		BEGIN
			
			ROLLBACK TRAN 
			RAISERROR ('An error ocurred during deleting the code associated with the user', 16, 1)
			RETURN -1

		END
		
		--Delete all entries in the [dbo].[Users] table:
		DELETE FROM [dbo].[Users]
		 WHERE [UserId] = @userId	
		
		-- Get the value of the @@ERROR global variable:
		SET @error = @@ERROR

		IF (@error != 0)
		BEGIN
			
			ROLLBACK TRAN 
			RAISERROR ('An error ocurred during deleting the user', 16, 1)
			RETURN -1

		END

		-- There are no errors, so commit the transaction:
		COMMIT TRAN
		
		RETURN 0

	END


END
go
