IF OBJECT_ID('dbo.roc_spGetSectionsAndProductsByFullText') IS NOT NULL
	DROP PROCEDURE dbo.roc_spGetSectionsAndProductsByFullText
go
...
/* 
	...
	EXECUTE dbo.roc_spGetSectionsAndProductsByFullText @editionId = 3, @parentId = 3, @fulltext = 'grasas',  @numberByPage = 10 , @page = 0


 */ 

CREATE PROCEDURE dbo.roc_spGetSectionsAndProductsByFullText
(
    @editionId		int,
	@parentId		int,
	@fulltext		varchar(30),
 ...
 BEGIN
  ...
  SELECT(
    SELECT count(*) FROM
		...
		(FREETEXT(SectionName,@fulltext) or FREETEXT(ProductName, @fulltext))
		)as contador
		)as total,*
		from(
