IF OBJECT_ID('dbo.plm_spGetPresentationsByEditionByProduct') IS NOT NULL
	DROP PROCEDURE dbo.plm_spGetPresentationsByEditionByProduct
go

/* 
	Author:			Ramiro Sánchez
	Object:			dbo.plm_spGetPresentationsByEditionByProduct
	
	Description:	Gets all presentations by product.
	Company:		PLM Latina.

	EXECUTE dbo.plm_spGetPresentationsByEditionByProduct @editionId = 55, @divisionId = 3, @categoryId = 103, @productId = 6660, @pharmaFormId = 210

 */ 

CREATE PROCEDURE dbo.plm_spGetPresentationsByEditionByProduct
(
	@editionId		int,
	@divisionId		int,
	@categoryId		int,
...
BEGIN
	...
	From EditionPresentations epr
		Inner Join Presentations p On epr.PresentationId = p.PresentationId
