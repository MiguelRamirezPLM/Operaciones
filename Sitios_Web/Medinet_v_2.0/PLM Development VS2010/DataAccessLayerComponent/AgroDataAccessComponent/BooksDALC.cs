...
{
    public sealed class BooksDALC : AgroDataAccessAdapter<BooksInfo>
    {
        #region Constructors
        ...
        public List<BooksInfo> getAllByCountry(int countryId)
        {
            ...
            using (IDataReader dataReader = BooksDALC.AgroInstanceDatabase.ExecuteReader(
                BooksDALC.AgroInstanceDatabase.GetStoredProcCommand("dbo.plm_spGetBooksByCountry", countryId)))
            {
        ...
        public List<BooksInfo> getEncyclopediaBooksByCountry(int countryId)
        {
            ...
            // Get the resultset from the stored procedure:
            using (IDataReader dataReader = BooksDALC.AgroInstanceDatabase.ExecuteReader(
                BooksDALC.AgroInstanceDatabase.GetStoredProcCommand("dbo.plm_spGetEncyclopediaBooksByCountry", countryId)))
            {
                // Iterates through records:
                while (dataReader.Read())
                    BECollection.Add(this.getFromDataReader(dataReader));
        ...
        public override BooksInfo getOne(int pk)
        {
            ...
            DbCommand dbCmd = BooksDALC.AgroInstanceDatabase.GetStoredProcCommand("dbo.plm_spCRUDBooks");

            // Add the parameters:
            BooksDALC.AgroInstanceDatabase.AddParameter(dbCmd, "@CRUDType", DbType.Byte,
                ParameterDirection.Input, string.Empty, DataRowVersion.Current, CRUD.Read);
            BooksDALC.AgroInstanceDatabase.AddParameter(dbCmd, "@bookId", DbType.Int32,
                ParameterDirection.Input, string.Empty, DataRowVersion.Current, pk);

            // Get the result set from the stored procedure:
            using (IDataReader dataReader = BooksDALC.AgroInstanceDatabase.ExecuteReader(dbCmd))
            {
                if (dataReader.Read())
