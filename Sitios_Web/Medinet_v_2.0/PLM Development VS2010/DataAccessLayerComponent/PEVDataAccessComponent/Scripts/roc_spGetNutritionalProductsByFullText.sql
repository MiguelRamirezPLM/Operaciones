IF OBJECT_ID('dbo.roc_spGetNutritionalProductsByFullText') IS NOT NULL
   DROP PROCEDURE  dbo.roc_spGetNutritionalProductsByFullText
...
go
  ...
  /*
	 ...
	 Object:	   dbo.roc_spGetNutritionalProductsByFullText
	
	Description:	Retrieves all  products nutritional for full text

	Company:		PLM México.

    EXECUTE dbo.roc_spGetNutritionalProductsByFullText @editionId=1, @countryId=10, @text='ACTIFLOR LACTOMIL',  @page=0, @numberpage=10
                                                             

*/

CREATE PROCEDURE dbo.roc_spGetNutritionalProductsByFullText
(
  @editionid         int,
  @countryid         int,
  @text              varchar(250),
