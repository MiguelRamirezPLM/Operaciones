IF OBJECT_ID('dbo.roc_spGetProductsByCombinedActiveSubstance') IS NOT NULL
	DROP PROCEDURE dbo.roc_spGetProductsByCombinedActiveSubstance
go

/* 
	Author:			Daniel Campos / Ramiro Sánchez				 
	Object:			dbo.roc_spGetProductsByCombinedActiveSubstance
	
	Description:	Retrieves all Products by Edition and ActiveSubstance.

	Company:		PLM Latina.

	EXECUTE dbo.roc_spGetProductsByCombinedActiveSubstance @activeSubstanceId = 123, @editionId = 6

 */ 

CREATE PROCEDURE dbo.roc_spGetProductsByCombinedActiveSubstance
(
	...
	SELECT DISTINCT (P.ProductId), P.ProductName, P.Description, P.Register, PF.PharmaFormId,
		...
		WHERE ASU.ActiveSubstanceId = @activeSubstanceId 
			...
			AND (SELECT COUNT(*) FROM ProductSubstances 
				...
				WHERE  ProductSubstances.ProductId = P.ProductId  AND ActiveSubstances.Active = 1) > 1
