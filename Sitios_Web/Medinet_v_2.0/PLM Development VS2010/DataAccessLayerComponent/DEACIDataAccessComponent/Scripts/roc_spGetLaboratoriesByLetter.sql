IF OBJECT_ID('dbo.roc_spGetLaboratoriesByLetter') IS NOT NULL
	DROP PROCEDURE dbo.roc_spGetLaboratoriesByLetter
go

/* 
	Author:			Daniel Campos. / Elizabeth Lazo.				 
	Object:			dbo.roc_spGetLaboratoriesByLetter
	
	Description:	Retrieves all laboratories by letter.

	Company:		PLM Latina.

	EXECUTE dbo.roc_spGetLaboratoriesByLetter @page=0, @numberByPage= 10,  @editionId = 2, @companyTypeId =5, @letter = 'q'


 */ 

CREATE PROCEDURE dbo.roc_spGetLaboratoriesByLetter
(   
  @page               int,   
  @numberByPage       int,
  @editionId          int,
...
AS
  ...
  --The parameters shouldn't be null:
       IF (@page IS NULL OR @numberByPage IS NULL OR  @editionId IS NULL
            OR @companyTypeId IS NULL OR @letter IS NULL) 
