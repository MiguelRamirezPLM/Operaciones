...
namespace DEAQBusinessLogicComponent
{
    public class ProductsBLC
    {
        ...
        public List<ProductsToEditInfo> getEditableProductsByDivision(int divisionId, int countryId, int editionId, int bookId, string brand)
        {
            ...
            List<ProductsToEditInfo> products = DEAQDataAccessComponent.ProductsDALC.Instance.getEditableProductsByDivision(divisionId, countryId, editionId, bookId, brand);
            foreach (ProductsToEditInfo item in products)
            {
                List<DEAQBusinessEntries.CountryCodeInfo> countries = DEAQDataAccessComponent.CountriesDALC.Instance.getCountryCodesByProduct(Convert.ToInt32(item.ProductId), Convert.ToInt32(item.PharmaFormId), countryId);
                string countriesCodes = "";
                foreach (DEAQBusinessEntries.CountryCodeInfo coun in countries)
                {
