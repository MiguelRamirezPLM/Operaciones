...
namespace AgroBusinessLogicComponent
{
    public class AgrochemicalUsesBLC
    {
        ...
        public void addAgrochemicalUsesForProduct(int productId, int agrochemicalUseId, int userId, string hash)
        {
            ...
            activityLog.TableId = Convert.ToInt32(PLMUsersBusinessLogicComponent.PLMUsersBusinessLogic.Tables.ProductAgrochemicalUses);
        ...
        public List<AgroBusinessEntries.AgrochemicalUseInfo> getAgrochemicalUsesNotAssociated(int editionId, int divisionId, int bookId,int productId, int pharmaformId, string brand)
        {
            if (editionId <= 0 || divisionId <= 0)
                throw new ArgumentException("The Edition or Country does not exist.");

            return AgroDataAccessComponent.AgrochemicalUsesDALC.Instance.getAgrochemicalUsesNotAssociated(editionId, divisionId, bookId, productId, pharmaformId,  brand);
        }
