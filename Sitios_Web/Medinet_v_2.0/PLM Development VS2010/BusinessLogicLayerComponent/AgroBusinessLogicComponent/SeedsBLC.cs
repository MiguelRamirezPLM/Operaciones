...
namespace AgroBusinessLogicComponent
{
    public  class SeedsBLC
    {
        ...
        public void addSeedsForProduct(int productId, int agrochemicalUseId, int userId, string hash)
        {
            ...
            activityLog.TableId = Convert.ToInt32(PLMUsersBusinessLogicComponent.PLMUsersBusinessLogic.Tables.ProductSeeds);
            ...
            activityLog.FieldsAffected = "(SeedId," + agrochemicalUseId + ");";
        ...
        public List<AgroBusinessEntries.SeedDetailInfo> getSeedsNotAssociated(int editionId, int divisionId, int bookId, int productId, int pharmaformId, string brand)
        {
            if (editionId <= 0 || divisionId <= 0)
                throw new ArgumentException("The Edition or Country does not exist.");

            return AgroDataAccessComponent.SeedsDALC.Instance.getSeedsNotAssociated(editionId, divisionId, bookId, productId, pharmaformId, brand);
        }
