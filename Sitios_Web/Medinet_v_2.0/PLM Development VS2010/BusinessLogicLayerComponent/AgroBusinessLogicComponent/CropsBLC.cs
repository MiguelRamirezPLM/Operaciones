   ...
   public class CropsBLC
    {
       ...
       public void addCropsForProduct(int productId, int agrochemicalUseId, int userId, string hash)
       {
           ...
           activityLog.TableId = Convert.ToInt32(PLMUsersBusinessLogicComponent.PLMUsersBusinessLogic.Tables.ProductCrops);
           ...
           activityLog.FieldsAffected = "(CropId," + agrochemicalUseId + ");";
       ...
       public List<AgroBusinessEntries.CropDetailInfo> getCropsNotAssociated(int editionId, int divisionId, int bookId, int productId, int pharmaformId, string brand)
       {
           if (editionId <= 0 || divisionId <= 0)
               throw new ArgumentException("The Edition or Country does not exist.");

           return AgroDataAccessComponent.CropsDALC.Instance.getCropsNotAssociated(editionId, divisionId, bookId, productId, pharmaformId, brand);
       }
