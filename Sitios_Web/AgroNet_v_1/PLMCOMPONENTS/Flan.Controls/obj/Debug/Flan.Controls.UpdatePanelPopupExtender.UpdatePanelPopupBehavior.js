...
Flan.UpdatePanelPopupBehavior = function(element) { 
    Flan.UpdatePanelPopupBehavior.initializeBase(this, [element]);
...
Flan.UpdatePanelPopupBehavior.prototype = {
    initialize : function() {
        Flan.UpdatePanelPopupBehavior.callBaseMethod(this, 'initialize');
        this._addHandlers();
    ...
    dispose : function() {
        this._clearHandlers();
        Flan.UpdatePanelPopupBehavior.callBaseMethod(this, 'dispose');
    },
    //
    // Public methods
    //
    ...
    _positionEltLocation : function() {
        ...
        switch(this._align) {
            case Flan.HorizontalAlign.Left :
                break;
            case Flan.HorizontalAlign.Center :
                x += parseInt(bounds.width /2);
                break;
            case Flan.HorizontalAlign.Right :
                x += bounds.width;
                break;
        }
        ...
        switch(this._valign) {
             case Flan.VerticalAlign.Middle :
                y += parseInt(bounds.height /2);
                break;
            case Flan.VerticalAlign.Top :
                break;
            case Flan.VerticalAlign.Bottom :
                y += bounds.height;
                break;
        }
            ...
            for (var i = 0; i < this._calloutWidth; i++) {
                ...
                for (var j = 0; j < this._calloutWidth; j++) {
                    ...
                    if (j > i - 1) {
                        ...
                        if (this._calloutType === Flan.CalloutType.TransparentGradient) {
...
Flan.UpdatePanelPopupBehavior.registerClass('Flan.UpdatePanelPopupBehavior', Sys.UI.Behavior);

//Enums

Flan.HorizontalAlign = function(){
	throw Error.invalidOperation();
};
Flan.HorizontalAlign.prototype = 
{
    Right  : 0,
    Center : 1,
    Left   : 2
}
Flan.HorizontalAlign.registerEnum('Flan.HorizontalAlign');


Flan.VerticalAlign = function(){
	throw Error.invalidOperation();
};
Flan.VerticalAlign.prototype = 
{
    Middle : 0,
    Top    : 1,
    Bottom : 2
}
Flan.VerticalAlign.registerEnum('Flan.VerticalAlign');

Flan.CalloutType = function(){
	throw Error.invalidOperation();
};
Flan.CalloutType.prototype = 
{
    TransparentGradient : 0,
    Solid    : 1
}
Flan.CalloutType.registerEnum('Flan.CalloutType');