...
function createproduct(value) {
    ...
    var div = $("#DivisionId").val();
    var edition = $("#EditionId").val();
    var register = $("#Registry").val();
    var count = $("#CountryId").val();
    var d = "";
    ...
    if ((!name.trim() == false) && (pf != "0") && (cat != "0")) {
        $.ajax({
            ...
            data: {
                ...
                pharmaform: pf,
                category: cat,
                Mentionated: MentProd,
                Participant: PartProd,
                RegisterSanitary: register,
                Country: count
            },
            success: function (data) {
                if (data == true) {
...
function close_popup_createproduct() {
    ...
    $("#Description").val('');
    $("#Registry").val('');
    $("#PForm").val(document.getElementById("0"));
    $("#Category").val(document.getElementById("0"))
...
function open_popup_createaddress() {
    $('#insertaddressSMD').fadeIn('slow');
    $('.popup-overlay').fadeIn('slow');
    $('.popup-overlay').height($(window).height());
    return false;
...
function close_popup_createaddress() {
    $('#insertaddressSMD').fadeOut('slow');
    $('.popup-overlay').fadeOut('slow');
    $("#bloqueo").hide();
