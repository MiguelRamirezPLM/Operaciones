...
namespace MediVet.Models
{
    ...
    public partial class Users
    {
        ...
        public bool IsValid(string _Nname, string _Password)
        {
            ...
            var login = db.Users.Where(x => x.NickName == _Nname && x.Password == _Password).ToList();
                ...
                foreach (Users User in login)
                {
                    ...
                    if (Applic.LongCount() > 0)
                    {
                        foreach (Applications Ap in Applic)
                        {
                            ...
                            var AppUser = (from Appu in db.ApplicationUsers
                                           where Appu.UserId == UserId
                                           && Appu.ApplicationId == Applications.ApplicationId
                                           select Appu).ToList();
                                    ...
                                    if (Roles.LongCount() > 0)
                                    {
                                        foreach (Roles Rol in Roles)
                                        {
                                            ...
                                            var Count = (from CountryU in db.CountriesUser
                                                         where CountryU.CountryId == CountryId
                                                         select CountryU).ToList();
